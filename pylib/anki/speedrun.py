...
def _build_filtered(
    col: Collection, name: str, search: str, limit: int, order: int
) -> DeckId:
...
def build_timed_test(
    ...
    return _build_filtered(col, TIMED_DECK_NAME, _SEARCH, questions, 1)
...
def build_review_deck(col: Collection, cids: list[int]) -> DeckId | None:
    ...
    # DUE = 6 keeps a stable order; limit covers all supplied cards.
    return _build_filtered(col, REVIEW_DECK_NAME, search, len(cids), 6)
